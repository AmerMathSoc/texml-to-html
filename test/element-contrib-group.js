...
tape('Template: contrib-group', async function(t) {
  ...
  t.equal(contribgroup.innerHTML, 'ContribA Information', 'contrib-group creates dt with content derived from @content-type'); // NOTE last char stripped | TODO should we check for actually expected values?
  t.equal(contribgroup.nextElementSibling.tagName, 'DD', 'nextSibling is DD')
  t.equal(contribgroup.nextElementSibling.getAttribute('data-ams-doc-contrib'), 'contribAs', 'nextSibling has data-ams-doc-contrib with contrib-group@content-type');
