...
Some XML elements have the same name (and purpose) in HTML. We preserve them in the output:
...
Some XML attributes have the same name (and purpose) in HTML.

As per our general strategy, we only preseve some attributes on some elements.
...
- data-ams-doc
  ...
  - stringname
  - verse-group
  - label [on span wrapped around sec & statement label content, cf. #303]
...
The relevant components in `texml-to-html` (i.e., `article-metadata-json.js`, `book-meta-json.js`, `sec-meta.js`) should provide a (hopefully easey enough) overview how the XML metadata is mapped and stored. The snapshots in the `test` folder should also be helpful, alongside any (example) article's JSON blob.
...
For math mode, texml creates MathJax-optimized TeX strings that may contain XML markup (for content not supported by MathJax it falls back to SVG creation). This XML requires extra processing.

#### xref

Math mode output may contain xref elements. This creates `\xhref[@ref-type]{#@rid}{...}`; this custom MathJax macro works in both text and math mode.

#### text inside math mode

In the case where math mode contains text mode, texml creates text elements possibly containing text XML markup.
We adjust our processing to create MathJax-compatible TeX strings.
...
- nested math mode can only appear within text and will only be inline-formula which is converted to `$...$`
