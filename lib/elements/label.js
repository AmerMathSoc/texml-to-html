...
export default function (htmlParentNode, xmlnode) {
  ...
  // disp-formula@content-type=text (aka "thingy" environment)
  const isDispFormulaText = xmlnode.parentNode.tagName === 'disp-formula' && xmlnode.parentNode.getAttribute('content-type') === 'text';
  if (isDispFormulaText) {
    const labelSpan = this.createNode('span', '', { 'data-ams-doc': 'label' });
    htmlParentNode.appendChild(labelSpan);
    this.passThrough(labelSpan, xmlnode);
    return;
  }
