 ...
 */
const generateBookJson = (recurseTheDom, passThrough, createNode, bookMetaNode) => {
    ...
    const extractContribAffiliation = (contrib, xref) => {
        ...
        // TODO handle specific-use="current" (no use case)
        const span = createNode('span');
    ...
    const extractContrib = (bookMetaJsonContributorGroup, contrib) => {
        const contributor = {};
    ...
    // identifiers
    bookMetaJson.book.identifiers.AMS.publKey = bookMetaNode.querySelector('book-id[assigning-authority="AMS"][book-id-type="publ_key"]')?.textContent;
    ...
    contribGroups.forEach(contribGroup => {
        ...
        contribGroup.querySelectorAll('contrib').forEach(extractContrib.bind(null, bookMetaJson.contributors[groupType]))
    });
...
exports.generateBookJson = generateBookJson;
