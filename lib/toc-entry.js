module.exports = (recurseTheDom, passThrough, createNode) => {
  const tocEntry = (htmlParentNode, xmlnode) => {
    ...
    const label = [...xmlnode.children].find(child => child.tagName === 'label');
    const title = [...xmlnode.children].find(child => child.tagName === 'title');;
    const altTitle = [...xmlnode.children].find(child => child.tagName === 'alt-title');;
    if (label && label.innerHTML.trim() !== '') {
      passThrough(anchor, label);
      anchor.insertAdjacentText('beforeend', '. ');
    }
    if (altTitle) passThrough(anchor, altTitle);
    else if (title) passThrough(anchor, title);
