 ...
 * @param {Node} xmlnode 
 */
const wrapTeXMathInDollar = xmlnode => {
  [...xmlnode.children].filter(node => node.tagName === 'tex-math').forEach(texMath => {
...
module.exports = passThrough => {
  ...
  const text = (htmlParentNode, xmlnode) => {
    ...
    if (xmlnode.closest('tex-math')) {
      ...
      wrapTeXMathInDollar(xmlnode); // NOTE must run AFTER replaceTeXCharactersInNodes() so that the added $'s are not escaped
